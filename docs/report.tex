...
\begin{tikzpicture}[node distance = 3.2 cm, auto]
    ...
    \path[line] (top)   -- node {P(south \textbar \, B)} (center);
    \path[line] (left)  -- node {P(east \textbar \, B)} (center);
    \path[line] (right) -- node {P(west \textbar \, D)} (center);
    \path[line] (bottom) -- node {P(north \textbar \, W)} (center);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[node distance = 3.2 cm, auto]
    ...
    \path[line] (top)   -- node {P(south \textbar \, F)} (center);
    \path[line] (left)  -- node {P(east \textbar \, D)} (center);
    \path[line] (right) -- node {P(west \textbar \, edge)} (center);
    \path[line] (bottom) -- node {P(north \textbar \, F)} (center);
\end{tikzpicture}
\end{center}
