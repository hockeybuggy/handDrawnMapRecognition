...
\begin{tikzpicture}[node distance = 3.2 cm, auto]
    ...
    \path[line] (top)   -- node {$P_{south}(X | \, D)$} (center);
    \path[line] (left)  -- node {$P_{east}(X | \, F)$} (center);
    \path[line] (right) -- node {$P_{west}(X | \, G)$} (center);
    \path[line] (bottom) -- node {$P_{north}(X | \, G)$} (center);
\end{tikzpicture}
\end{center}
