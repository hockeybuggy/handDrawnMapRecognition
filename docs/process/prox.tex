...
\begin{tikzpicture}[node distance=3cm, auto]
    ...
    \path[line] (top)   -- node[above right] {$P(X | \, N=C_{i,j+1})$} (center);
    \path[line] (left)  -- node[above=.75cm] {$P(X | \, W=C_{i-1,j})$} (center);
    \path[line] (right) -- node[above=.75cm] {$P(X | \, E=C_{i+1,j})$} (center);
    \path[line] (bottom) -- node[below right] {$P(X | \, S=C_{i,j-1})$} (center);
\end{tikzpicture}
\end{center}
